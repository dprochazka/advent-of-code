...
def measure_universe(lines, factor):
    ...
    for g1, g2 in itertools.combinations(galaxies, 2):
        vertical_distance = abs(g1[0] - g2[0]) + (factor - 1) * len(
            set(range(min(g1[0], g2[0]), max(g1[0], g2[0]))) & eh
        )
        horizontal_distance = abs(g1[1] - g2[1]) + (factor - 1) * len(
            set(range(min(g1[1], g2[1]), max(g1[1], g2[1]))) & ev
        )
...
if __name__ == "__main__":
    ...
    print(measure_universe(lines, 2))
    print(measure_universe(lines, 10**6))
